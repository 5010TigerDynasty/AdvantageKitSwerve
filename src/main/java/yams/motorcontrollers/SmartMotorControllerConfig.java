...
public class SmartMotorControllerConfig {
  ...
  private MechanismGearing externalEncoderGearing =
      new MechanismGearing(new GearBox(new double[] {1.0}));
  ...
  /**
   * Set the feedback synchronization threshhold so the relative encoder synchronizes with the
  ...
  public SmartMotorControllerConfig withContinuousWrapping(Angle bottom, Angle top) {
    ...
    simpleController.ifPresent(
        pidController ->
            pidController.enableContinuousInput(bottom.in(Rotations), top.in(Rotations)));
    controller.ifPresent(
        profiledPIDController ->
            profiledPIDController.enableContinuousInput(bottom.in(Rotations), top.in(Rotations)));
  ...
  public SmartMotorControllerConfig withClosedLoopTolerance(Angle tolerance) {
    if (tolerance != null) {
      controller.ifPresent(
          profiledPIDController -> profiledPIDController.setTolerance(tolerance.in(Rotations)));
      simpleController.ifPresent(
  ...
  public SmartMotorControllerConfig withClosedLoopTolerance(Distance tolerance) {
    ...
    if (tolerance != null) {
      controller.ifPresent(
          profiledPIDController -> profiledPIDController.setTolerance(tolerance.in(Meters)));
   ...
   * @return {@link SmartMotorControllerConfig} for chaining.
   */
  public SmartMotorControllerConfig withSpecificTelemetry(
   ...
   *     followers, must be same brand as the {@link SmartMotorController} with inversion..
   * @return {@link SmartMotorControllerConfig} for chaining
   */
  @SafeVarargs
   ...
   * Rotations and outputs are in Rotations.
   *
   * @param controller {@link ProfiledPIDController} to use, the units passed in are in Rotations
   *     and output is Voltage.
   * @return {@link SmartMotorControllerConfig} for chaining.
   */
  ...
  public Optional<Angle> getDiscontinuityPoint() {
    if (maxDiscontinuityPoint.isPresent()
        && minDiscontinuityPoint.isPresent()
        && !minDiscontinuityPoint
            .get()
            .equals(Rotations.of(maxDiscontinuityPoint.get().in(Rotations) - 1))) {
      throw new SmartMotorControllerConfigurationException(
          "Bounds are not correct!",
          "Cannot get the discontinuity point.",
          "withContinuousWrapping(Rotations.of("
              + Rotations.of(maxDiscontinuityPoint.get().in(Rotations) - 1).in(Rotations)
              + "),Rotations.of("
              + maxDiscontinuityPoint.get().in(Rotations)
              + ")) instead ");
    }
