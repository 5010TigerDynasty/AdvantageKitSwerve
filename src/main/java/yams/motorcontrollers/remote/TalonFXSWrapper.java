...
public class TalonFXSWrapper extends SmartMotorController {
          ...
          Optional.of(
              new DCMotorSim(
                  LinearSystemId.createDCMotorSystem(
                      m_dcmotor, 0.001, config.getGearing().getRotorToMechanismRatio()),
                  m_dcmotor));
    }
  }
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (config.getClosedLoopController().isPresent()) {
      ...
      if (controller.getPositionTolerance()
          != new ProfiledPIDController(0, 0, 0, new Constraints(0, 0)).getPositionTolerance()) {
        throw new IllegalArgumentException(
      ...
      if (config.getMechanismCircumference().isPresent()) {
        ...
        m_talonConfig.MotionMagic.withMotionMagicCruiseVelocity(
            RotationsPerSecond.of(controller.getConstraints().maxAcceleration));
      }
      if (config.getMechanismCircumference().isPresent()) {
        m_talonConfig.MotionMagic.withMotionMagicAcceleration(
            config.convertToMechanism(
                MetersPerSecondPerSecond.of(controller.getConstraints().maxAcceleration)));
      } else {
        m_talonConfig.MotionMagic.withMotionMagicAcceleration(
            RotationsPerSecondPerSecond.of(controller.getConstraints().maxAcceleration));
      }
    ...
    if (config.getArmFeedforward().isPresent()
        || config.getElevatorFeedforward().isPresent()
    ...
    if (config.getExternalEncoder().isPresent()) {
      ...
      if (config.getExternalEncoder().get() instanceof CANcoder encoder) {
        ...
        if (encoder.getIsProLicensed().getValue()) {
          m_talonConfig.ExternalFeedback.ExternalFeedbackSensorSource =
              ExternalFeedbackSensorSourceValue.SyncCANcoder;
        } else {
          m_talonConfig.ExternalFeedback.ExternalFeedbackSensorSource =
        ...
        if (config.getZeroOffset().isPresent()) {
          cfg.MagnetSensor.withMagnetOffset(
              encoder
                  .getPosition()
                  .getValue()
                  .plus(Rotations.of(cfg.MagnetSensor.MagnetOffset))
        ...
        if (config.getDiscontinuityPoint().isPresent()) {
          cfg.MagnetSensor.withAbsoluteSensorDiscontinuityPoint(
              config.getDiscontinuityPoint().get());
        }
        configurator.apply(cfg);
        ...
        if (useCANdiPWM1()) {
          ...
          // Zero offset
          if (config.getZeroOffset().isPresent()) {
            cfg.PWM1.withAbsoluteSensorOffset(
                encoder
                    .getPWM1Position()
                    .getValue()
                    .plus(Rotations.of(cfg.PWM1.AbsoluteSensorOffset))
          ...
          // Zero offset
          if (config.getZeroOffset().isPresent()) {
            cfg.PWM2.withAbsoluteSensorOffset(
                encoder
                    .getPWM2Position()
                    .getValue()
                    .plus(Rotations.of(cfg.PWM2.AbsoluteSensorOffset))
      ...
      m_talonConfig.ExternalFeedback.RotorToSensorRatio =
          1.0; // config.getGearing().getRotorToMechanismRatio();
      m_talonConfig.ExternalFeedback.SensorToMechanismRatio =
          config.getGearing().getMechanismToRotorRatio();
      // Zero offset.
      if (config.getZeroOffset().isPresent()) {
      ...
      if (config.getDiscontinuityPoint().isPresent()) {
        ...
        m_talonConfig.ExternalFeedback.withAbsoluteSensorDiscontinuityPoint(
            config.getDiscontinuityPoint().get());
      }
    }
      ...
      for (Pair<Object, Boolean> follower : config.getFollowers().get()) {
        if (follower.getFirst() instanceof TalonFXS) {
          ((TalonFXS) follower.getFirst())
              .setControl(
                  new Follower(m_talonfxs.getDeviceID(), follower.getSecond())
                      .withUpdateFreqHz(config.getClosedLoopControlPeriod().in(Second)));
