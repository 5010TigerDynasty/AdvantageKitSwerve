...
public class SparkWrapper extends SmartMotorController {
    ...
    if (sparkAbsoluteEncoder.isPresent()) {
      ...
      sparkRelativeEncoderSim.ifPresent(
          sparkRelativeEncoderSim ->
              sparkRelativeEncoderSim.setPosition(sparkAbsoluteEncoder.get().getPosition()));
    }
  }
  ...
  public void synchronizeRelativeEncoder() {
    if (config.getFeedbackSynchronizationThreshold().isPresent()) {
      if (sparkAbsoluteEncoder.isPresent()) {
        if (!Rotations.of(sparkRelativeEncoder.getPosition())
            .isNear(
                Rotations.of(sparkAbsoluteEncoder.get().getPosition()),
                config.getFeedbackSynchronizationThreshold().get())) {
          seedRelativeEncoder();
        }
      }
    }
  ...
  public void simIterate(AngularVelocity mechanismVelocity) {
    if (RobotBase.isSimulation()) {
      sparkSim.ifPresent(
          sim ->
              sim.iterate(
                  mechanismVelocity.in(RotationsPerSecond),
      ...
      sparkAbsoluteEncoderSim.ifPresent(
          absoluteEncoderSim ->
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder instanceof SparkAbsoluteEncoder) {
        double absoluteEncoderConversionFactor =
            config.getExternalEncoderGearing().getRotorToMechanismRatio();
        ...
        sparkBaseConfig.absoluteEncoder.velocityConversionFactor(
            absoluteEncoderConversionFactor / 60);
      ...
      for (Pair<Object, Boolean> follower : config.getFollowers().get()) {
        if (follower.getFirst() instanceof SparkMax) {
          ((SparkMax) follower.getFirst())
              .configure(
                  new SparkMaxConfig().follow(spark, follower.getSecond()),
          ...
          ((SparkFlex) follower.getFirst())
              .configure(
                  new SparkFlexConfig().follow(spark, follower.getSecond()),
                  ResetMode.kNoResetSafeParameters,
                  PersistMode.kPersistParameters);
    ...
    if (config.getZeroOffset().isPresent()) {
      DriverStation.reportError(
          "[ERROR] Zero offset is not supported on Sparks, or we have not implemented this!",
          false);
    }
    ...
    return configureSpark(
        () ->
            spark.configure(
                sparkBaseConfig, ResetMode.kNoResetSafeParameters, PersistMode.kPersistParameters));
  }
  ...
  public Current getStatorCurrent() {
    return sparkSim
        .map(sim -> Amps.of(sim.getMotorCurrent()))
        .orElseGet(() -> Amps.of(spark.getOutputCurrent()));
  }
  ...
  public AngularVelocity getRotorVelocity() {
    return RotationsPerSecond.of(
        getMechanismPosition().in(Rotations) * config.getGearing().getMechanismToRotorRatio());
  }
