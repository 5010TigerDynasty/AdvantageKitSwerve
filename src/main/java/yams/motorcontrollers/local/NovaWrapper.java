...
public class NovaWrapper extends SmartMotorController {
          ...
          Optional.of(
              new DCMotorSim(
                  LinearSystemId.createDCMotorSystem(
                      m_motor, 0.001, config.getGearing().getRotorToMechanismRatio()),
                  m_motor,
                  1.0 / 1024.0));
    }
  }
  ...
  public void seedRelativeEncoder() {
    DriverStation.reportWarning(
        "[WARNING] NovaWrapper.seedRelativeEncoder() is not supported", false);
  }
  ...
  public void setEncoderVelocity(AngularVelocity velocity) {
    //    m_sim.ifPresent(dcMotorSim ->
    // dcMotorSim.setAngularVelocity(velocity.in(RadiansPerSecond)));
    DriverStation.reportWarning(
        "[WARNING] NovaWrapper.setEncoderVelocity() is not supported on ThriftyNova's.", false);
  }
  ...
  public boolean applyConfig(SmartMotorControllerConfig config) {
    ...
    if (config.getEncoderInverted()) {
      throw new IllegalArgumentException(
          "[ERROR] ThriftyNova does not support encoder inversions.");
    }
    ...
    if (config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder == EncoderType.QUAD) {
        ...
        throw new IllegalArgumentException(
            "[ERROR] Unsupported external encoder: "
                + externalEncoder.getClass().getSimpleName()
    ...
    if (config.getZeroOffset().isPresent()) {
      DriverStation.reportError(
          "[ERROR] ThriftyNova does not support zero offset, or we have not implemented this.",
          true);
    }
  ...
  public double getDutyCycle() {
    return m_sim
        .map(dcMotorSim -> dcMotorSim.getInputVoltage() / RoboRioSim.getVInVoltage())
  ...
  public Angle getMechanismPosition() {
    ...
    if (config.getUseExternalFeedback() && config.getExternalEncoder().isPresent()) {
      ...
      if (externalEncoder == EncoderType.ABS) {
        return Rotations.of(
            m_nova.getPositionAbs()
                * config.getExternalEncoderGearing().getRotorToMechanismRatio());
      } else if (externalEncoder == EncoderType.QUAD) {
        return Rotations.of(
            m_nova.getPositionQuad()
                * config.getExternalEncoderGearing().getRotorToMechanismRatio());
      }
    }
