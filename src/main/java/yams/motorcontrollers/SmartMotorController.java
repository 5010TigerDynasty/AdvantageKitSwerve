...
public abstract class SmartMotorController {
  ...
  public void iterateClosedLoopController() {
      ...
      if (config.getArmFeedforward().isPresent()) {
        pidOutputVoltage =
            pidController
                .get()
                .calculate(
                    getMechanismPosition().in(Rotations), setpointPosition.get().in(Rotations));
        feedforward =
            config
                .getArmFeedforward()
                .get()
                .calculateWithVelocities(
                    ...
                    pidController.get().getSetpoint().velocity);
        ...
        pidOutputVoltage =
            pidController
                ...
                .calculate(
                    ...
                    config.convertFromMechanism(setpointPosition.get()).in(Meters));
        feedforward =
            config
      ...
      if (setpointPosition.isPresent()) {
        pidOutputVoltage =
            simplePidController
                .get()
                .calculate(
                    setpointPosition.get().in(Rotations), setpointPosition.get().in(Rotations));
      } else if (setpointVelocity.isPresent()) {
        pidOutputVoltage =
            simplePidController.get().calculate(setpointVelocity.get().in(RotationsPerSecond));
      }
    }
    if (config.getMechanismUpperLimit().isPresent()) {
    ...
    if (config.getClosedLoopControllerMaximumVoltage().isPresent()) {
      ...
      //      telemetry.outputVoltage = MathUtil.clamp(telemetry.outputVoltage, -maximumVoltage,
      // maximumVoltage);
    }
  ...
  /**
   * Run the {@link SysIdRoutine} which runs to the maximum MEASUREMENT at the step voltage then
   * down to the minimum MEASUREMENT with the step voltage then up to the maximum MEASUREMENT
   * increasing each second by the step voltage generated via the {@link
   * SmartMotorControllerConfig}.
   *
   ...
   * @return Sequential command group of {@link SysIdRoutine} running all required tests to the
   *     configured MINIMUM and MAXIMUM MEASUREMENTS.
   */
  public SysIdRoutine sysId(
      Voltage maxVoltage, Velocity<VoltageUnit> stepVoltage, Time testDuration) {
    SysIdRoutine sysIdRoutine = null;
    if (config.getTelemetryName().isEmpty()) {
      throw new SmartMotorControllerConfigurationException(
          "Telemetry is undefined",
    ...
    if (config.getMechanismCircumference().isPresent()) {
      sysIdRoutine =
          new SysIdRoutine(
              new Config(stepVoltage, maxVoltage, testDuration),
      ...
      if (config.getTelemetryName().isPresent()) {
        ...
        if (config.getSmartControllerTelemetryConfig().isPresent()) {
          this.telemetry.setupTelemetry(
              this, telemetry, tuning, config.getSmartControllerTelemetryConfig().get());
        } else {
          this.telemetry.setupTelemetry(
              this,
              telemetry,
              ...
              new SmartMotorControllerTelemetryConfig()
                  .withTelemetryVerbosity(config.getVerbosity().orElse(TelemetryVerbosity.HIGH)));
        }
        updateTelemetry();
      }
    }
  ...
  public void updateTelemetry() {
    ...
    // TODO: Update PID, Feedforward, current limits, soft limits, ramp rate, motor inversion,
    // encoder inversion
  }
