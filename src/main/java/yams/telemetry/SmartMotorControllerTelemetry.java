      ...
      switch (bt.getField()) {
        ...
        case TemperatureLimit -> {
          cfg.getTemperatureCutoff()
              .ifPresent(temperatureCutoff -> bt.set(smc.getTemperature().gte(temperatureCutoff)));
        }
    ...
    for (Map.Entry<DoubleTelemetryField, DoubleTelemetry> entry : doubleFields.entrySet()) {
      ...
      switch (dt.getField()) {
        case SetpointPosition -> {
          smc.getMechanismPositionSetpoint()
              .ifPresent(
                  mechSetpoint ->
                      dt.set(
                          cfg.getMechanismCircumference().isPresent()
        ...
        case MeasurementVelocity -> {
          cfg.getMechanismCircumference()
              .ifPresent(circumference -> dt.set(smc.getMeasurementVelocity().in(MetersPerSecond)));
        }
      ...
      if (dt.tunable()) {
        switch (dt.getField()) {
          case SetpointPosition -> {
            cfg.getMechanismCircumference()
                .ifPresentOrElse(
                    circumference -> smartMotorController.setPosition(Meters.of(dt.get())),
                    () -> smartMotorController.setPosition(Radians.of(dt.get())));
          }
          case SetpointVelocity -> {
            ...
            cfg.getMechanismCircumference()
                .ifPresentOrElse(
                    circumference -> smartMotorController.setVelocity(MetersPerSecond.of(dt.get())),
                    () ->
