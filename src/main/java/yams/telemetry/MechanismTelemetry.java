...
public class MechanismTelemetry {
  ...
  public void setupTelemetry(
    ...
    tunableSetpointSubscriber =
        tunableSetpointTopic.subscribe(
            convertToNativeUnit(position),
    ...
    motorController.setupTelemetry(
        networkTable.getSubTable(motorName), tuningNetworkTable.getSubTable(motorName));
  }
  ...
  private double convertToNativeUnit(Measure<?> unit) {
    return switch (unitsString) {
      ...
      default -> throw new IllegalArgumentException(
          "Cannot convert "
              + unit.toLongString()
              + " to double! Invalid unit given to mechanism telemetry!");
    };
  }
    ...
    return switch (unitsString) {
      ...
      default -> throw new IllegalArgumentException(
          "Cannot convert "
              ...
              + " to "
              + unitsString
              + "! Invalid unit given to mechanism telemetry!");
    };
  }

  /**
   * Checks if the setpoint in NetworkTables is different from the setpoint in the class, if it is
   * then the setpoint in NT has been updated and should be used instead of the requested setpoint.
   *
   * @return
   */
