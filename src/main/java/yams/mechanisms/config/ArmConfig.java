...
public class ArmConfig {
  ...
  /**
   * Lower Hard Limit for the {@link yams.mechanisms.positional.Arm} to be representing in
   * simulation.
   */
  ...
  /**
   * {@link yams.mechanisms.positional.Arm} MOI from CAD software. If not given estimated with
   * length and weight.
   */
   ...
   *
   * @param motorController Primary {@link SmartMotorController} for the {@link
   ...
   * yams.mechanisms.positional.Arm} for simulation.
   *
   ...
   */
  @Deprecated
  public ArmConfig withTelemetry(
      String networkRoot, String telemetryName, TelemetryVerbosity telemetryVerbosity) {
  ...
  /**
   * Wrap the arm around these angles. When the arm exceeds the maximum angle it will read as if it
  ...
  public double getMOI() {
    ...
    if (length.isPresent() && weight.isPresent()) {
      return SingleJointedArmSim.estimateMOI(
          length.get().in(Units.Meters), weight.get().in(Units.Kilograms));
    }
    throw new ArmConfigurationException(
        "Arm length and weight or MOI must be set!",
        "Cannot get the MOI!",
