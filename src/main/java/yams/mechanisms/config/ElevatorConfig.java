...
public class ElevatorConfig {
  ...
  /**
   * Lower Hard Limit for the {@link yams.mechanisms.positional.Elevator} to be representing in
   * simulation.
   */
  private Optional<Distance> lowerHardLimit = Optional.empty();
  /**
   * Upper hard limit for the {@link yams.mechanisms.positional.Elevator} representing in
   * simulation.
   */
   ...
   */
  @Deprecated
  public ElevatorConfig withTelemetry(
  ...
  public Optional<Distance> getStartingHeight() {
    return motor.getConfig().getStartingPosition().isPresent()
        ? Optional.of(
            motor.getConfig().convertFromMechanism(motor.getConfig().getStartingPosition().get()))
        : Optional.empty();
  }
