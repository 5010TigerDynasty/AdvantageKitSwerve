...
public abstract class SmartPositionalMechanism extends SmartMechanism {
  ...
  /**
   * Create the SysId routine and commands to run the SysId tests. The SysId test will run the
   * mechanism up then down at a constant speed, then run the mechanism up at an increasing speed
   * and down at an increasing speed. Requires the maximum and minimum limit to be set. Runs the
   * mechanism within 1 degree of the maximum and minimum.
   *
   * @param maximumVoltage Maximum {@link Voltage} to give to the {@link SmartPositionalMechanism},
   *     is the voltage given to run the {@link SmartPositionalMechanism} up at a static speed.
   * @param step Step {@link Voltage} to give to the {@link SmartPositionalMechanism}.
   * @param duration SysId test duration.
   * @return {@link edu.wpi.first.wpilibj2.command.SequentialCommandGroup} running the SysId
   *     commands.
   */
