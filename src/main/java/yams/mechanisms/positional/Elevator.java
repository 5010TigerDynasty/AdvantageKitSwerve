...
public class Elevator extends SmartPositionalMechanism {
    ...
    if (RobotBase.isSimulation()) {
      ...
      if (config.getCarriageMass().isEmpty()) {
        throw new ElevatorConfigurationException(
            "Mass is not configured!", "Cannot create simulator", "withMass(Mass)");
      }
      ...
      if (m_motor.getConfig().getMechanismLowerLimit().isPresent()) {
        mechanismWindow
            .getRoot("MinSoft", config.getMaximumHeight().get().minus(Inches.of(6)).in(Meters), 0)
            .append(
      ...
      if (m_motor.getConfig().getMechanismUpperLimit().isPresent()) {
        mechanismWindow
            .getRoot("MaxSoft", config.getMaximumHeight().get().plus(Inches.of(6)).in(Meters), 0)
            .append(
                new MechanismLigament2d(
                    "Limit",
                    m_motor
      ...
      mechanismWindow
          .getRoot("MinHard", config.getMaximumHeight().get().minus(Inches.of(8)).in(Meters), 0)
          .append(
              new MechanismLigament2d(
                  "Limit",
                  config.getMinimumHeight().get().in(Meters),
      ...
      mechanismWindow
          .getRoot("MaxHard", config.getMaximumHeight().get().plus(Inches.of(8)).in(Meters), 0)
          .append(
              new MechanismLigament2d(
                  "Limit",
                  config.getMaximumHeight().get().in(Meters),
      ...
      mechanismRoot =
          mechanismWindow.getRoot(
              config.getTelemetryName().isPresent()
                  ? config.getTelemetryName().get() + "Root"
                  : "ElevatorRoot",
              config.getMaximumHeight().get().in(Meters),
              0);
      mechanismLigament =
      ...
      SmartDashboard.putData(
          config.getTelemetryName().isPresent()
              ? config.getTelemetryName().get() + "/mechanism"
              : "Elevator/mechanism",
          mechanismWindow);
    }
  }
  ...
  public void updateTelemetry() {
    ...
    m_motor
        ...
        .ifPresent(m_setpoint -> m_telemetry.updateSetpoint(m_setpoint));
    ...
    if (m_sim.isPresent()) {
      ...
      if (m_config.getMinimumHeight().isPresent()
        ...
        RoboRioSim.setVInVoltage(
            BatterySim.calculateDefaultBatteryLoadedVoltage(m_sim.get().getCurrentDrawAmps()));
      }
      visualizationUpdate();
    }
  }
  ...
  public Translation3d getRelativeMechanismPosition() {
    ...
    Translation3d mechanismTranslation =
        new Translation3d(
            mechanismLigament.getLength(),
            new Rotation3d(
                Plane.YZ == movementPlane ? mechanismLigament.getAngle() : 0,
                Plane.XZ == movementPlane ? mechanismLigament.getAngle() : 0,
                0));
  ...
  public Command sysId(Voltage maximumVoltage, Velocity<VoltageUnit> step, Time duration) {
    ...
    if (m_config.getTelemetryName().isPresent()) {
      group =
          group.andThen(Commands.print(m_config.getTelemetryName().get() + " SysId test done."));
    }
    return group;
  }
